...
- `KIT-001` `PERF-001` `SEC-001..007` `COMPAT-001` 各领域
- `CSPRNG-001` `Math.random()` 在加密 / nonce / IV / signature 上下文（三档严重度：`/security/` 或 `/crypto/` 路径下 = High；文件含 `cryptoFramework` / `nonce` / `aesGcm` / `huks` / `signKey` / `hmac` 等关键字 = High；否则 Medium 提醒）。改用 `cryptoFramework.createRandom().generateRandomSync(N)` 或 HUKS

被钩子命中违规时，回复结构应是：**引用 ID → 引用 references 中的"正确写法" → 不要另造一个看似对的版本**。

## ArkTS-Sta（`use static` 静态模式）· 只需知情

HDC 2026 前后官方在推进 **ArkTS-Sta**（静态类型模式）：文件首行 `use static` 导语、真静态类型系统、对象天然共享（不再需要 `@Sendable`）、Native 互操作走 ANI（替代 NAPI）。**当前状态：演进中，生产不用。**

- 本 skill 的全部规则针对**动态 ArkTS**（当前生产主线，`.ets` 默认形态）
- 看到首行 `"use static"` 的文件：那是另一套类型/并发/状态管理语义，**不要**把本仓 V1/V2 状态规则与 NAPI 惯例套上去；提示用户该特性未到生产推荐阶段
- 不要主动建议用户迁移到 ArkTS-Sta
...
- 实战 case study：[`docs/case-studies/llm-chat-app.md`](../../../docs/case-studies/llm-chat-app.md) —— 真实 LLM 对话 app 的 ArkTS 踩坑全记录（M3 Configuration 命名空间 / M5 union content / M6 await-try / M9 资源句柄 / M11 弃用 API 等）
