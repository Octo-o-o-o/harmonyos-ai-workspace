...
## 多 host / 多 workspace 场景：单例 store 必须按 serverId 隔离

对于"per-host" / "per-workspace" 语义的全局单例 store（如 daemon 客户端的 DraftStore / CheckoutStore / TabsStore），**key 必须以 serverId 为前缀联合**，否则切 host 时状态串扰：

```typescript
// ❌ 单 key 分桶 — 两 host 同 cwd 互相覆盖
class CheckoutStoreImpl {
  private entries: Map<string, CheckoutEntry> = new Map();    // key = cwd
  setStatus(cwd: string, ...): void { this.entries.set(cwd, ...) }
}

// ✅ serverId + cwd 联合 key
class CheckoutStoreImpl {
  private entries: Map<string, CheckoutEntry> = new Map();    // key = `${serverId}:${cwd}`
  private keyOf(serverId: string, cwd: string): string { return serverId + ':' + cwd; }
  setStatus(serverId: string, cwd: string, ...): void {
    this.entries.set(this.keyOf(serverId, cwd), ...);
  }
  // host 断开时清掉本 host 全部 entry
  removeAllForHost(serverId: string): void { /* ... */ }
}
```
...
## Reducer 模式：所有写入封死单一入口

如果项目有 reducer / action 层（如 LLM 客户端的 `TimelineReducer` 处理 `agent_stream` 增量 + `fetch_agent_timeline` 历史拉取），**所有 store 写入必须经过 reducer**，避免绕路径导致元数据（timestamp / status 衍生 / race 保护）丢失：

```typescript
// ❌ 在调用方直接 append（绕过 reducer 内的 injectTimestamp）
const payload = await client.fetchAgentTimeline({ agentId });
for (const e of payload.entries) {
  this.session.appendTimelineItem(agentId, e.item);   // ← 跳过 timestamp 注入
}
...
Code review 时凡看到 `store.append*` / `store.set*` 直接调，先质疑是否绕开了 reducer。详见规则 `STATE-011`。
...
- 安卓上游对齐 case study：[`docs/case-studies/android-parity-migration.md`](../../../docs/case-studies/android-parity-migration.md) —— per-host store 隔离 / reducer path / cwd fallback / Codex review 流程
