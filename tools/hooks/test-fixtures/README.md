...
# 全部 11 个 .ets fixture 一次过（每个文件期望的 exit code 不一样）
for f in BadState BadArkTS BadDecorators BadSecurityKit BadRuntimePitfalls \
         InlineDecorators CustomDialogState ReusableState GoodPrefStore \
         BadHmsScanKit GoodHmsScanKit; do
  bash tools/hooks/lib/scan-arkts.sh "tools/hooks/test-fixtures/${f}.ets" >/dev/null 2>&1
  rc=$?
  case "$f" in
    GoodPrefStore|GoodHmsScanKit) expect=0 ;;
    *)                            expect=2 ;;
  esac
...
4. 把上面的"全部 N 个"循环 case 表更新 + 跑一遍全绿
