...
**接线层陷阱**（scanner 抓不到、灰度才暴露的）单独整理在 [`05-best-practices/bridge-integration-pitfalls.md`](05-best-practices/bridge-integration-pitfalls.md)：15 类常见 Web Bridge + 原生外壳集成坑（capability 握手 fail-closed、`javaScriptProxy` 生命周期、idempotencyKey 强制、envelope schema validation、token / SecureStore 写入原子性、pasteboard 提示时机、App-Linking 双侧配置 + EntryAbility 双入口、HMS ScanKit dual-import、原生 blur 有界增强、WebView 前后台生命周期统一管理、ArkWeb data-URL 下载等），沉淀自下游真工程教训。
