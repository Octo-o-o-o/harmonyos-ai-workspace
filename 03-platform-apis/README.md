...
| 原生 TCP/WebSocket | `@kit.NetworkKit` 的 `socket` / `webSocket` | 仅限明确 native protocol 场景；WebView companion 不应让 page world 驱动 raw socket |
...
### 3.1.1 Raw socket 边界（IMAP / SMTP / CalDAV 等）

`@kit.NetworkKit` 提供 socket 能力，但不要把它当成"移动端直接复刻服务端 provider"的默认答案。
在 Native Shell + WebView / enterprise companion 架构里，邮件和日历这类第三方协议应优先走服务端
HTTPS relay：端侧只收集用户显式输入并调用 `/api/.../bind`，raw IMAP / SMTP / CalDAV 连接、SSRF
防护、TLS 策略、附件大小限制、连接池和增量同步都放服务端。

端侧直接开 raw socket 只适合产品本身就是原生协议客户端，并且必须同时满足：

- host allowlist / 用户确认，拒绝 private、loopback、link-local、metadata IP；
- TLS-only 默认，STARTTLS 需要单独做 downgrade 防护；
- 凭据只进 HUKS-backed SecureStore，不经 WebView page world；
- timeout、附件大小上限、日志脱敏和真账号 smoke 作为发布门禁。

更完整的 WebView 边界见 `05-best-practices/bridge-integration-pitfalls.md` §17。
