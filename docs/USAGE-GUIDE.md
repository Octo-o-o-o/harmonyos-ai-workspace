...
**Codex CLI 同理**——在 app 根放 `AGENTS.md`，并保留 `.agents/skills/`。Codex 默认会从 git root 向上查找 `AGENTS.md`，项目级 skills 会从 `.agents/skills/` 自动发现；MCP-HarmonyOS 需要显式运行 `bash tools/setup-codex-mcp.sh` 写入用户级 Codex 配置；全局兜底可用 `~/.codex/AGENTS.md`。
