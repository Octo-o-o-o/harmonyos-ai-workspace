...
## M13 · 真机部署 · layered icon 幽灵 + 9568297 install 失败

> 此节是 v0.4.4 发布后的运维期反馈（非原始 M3-M12 里程碑），保留 M13 编号方便引用。

### 症状一 · 桌面图标叠加幽灵

把品牌 icon（蓝色对话气泡 + 锁）替换进 `products/<name>/src/main/resources/base/media/background.png` 后装机，桌面渲染出来锁的位置能隐约看到 DevEco 默认 startIcon 的"四方格"轮廓——新图标和旧默认图标叠加显示。

### 排查
- DevTools 看不出来：Read 工具预览 `foreground.png` 是空白白图
- 用 PIL 读像素：默认模板 `foreground.png` 是「半透明白色」，37.6% 像素 alpha≠0（RGB=255, alpha≈128–230）
- 也忘了改 `AppScope/resources/base/media/` 那一份——AppGallery / 任务管理器仍用 AppScope 资源

### 修复 diff

```diff
# 1) 两处 background.png 同步替换
- AppScope/resources/base/media/background.png            # 默认蓝渐变
+ AppScope/resources/base/media/background.png            # 完整品牌 icon 1024×1024
- products/<name>/src/main/resources/base/media/background.png
+ products/<name>/src/main/resources/base/media/background.png   # 同上

# 2) 两处 foreground.png 重新生成为真透明（alpha=0）
# 用 PIL：Image.new('RGBA', (1024,1024), (0,0,0,0)).save(p)

# 3) startIcon.png（启动窗口图，单层）
- products/<name>/src/main/resources/base/media/startIcon.png   # 144×144 模板四方格
+ products/<name>/src/main/resources/base/media/startIcon.png   # 256×256 品牌 icon
```

### 症状二 · `hdc install` 9568297

```
[Info]App install path:.../default-default-signed.hap
msg:error: failed to install bundle. code:9568297
error: install failed due to older sdk version in the device.
```

设备 MLR-AL10 跑 HarmonyOS 6.1.0（API 23），HAP 是用 `compatibleSdkVersion: "6.1.1(24)"` 打的。
...
+ "compatibleSdkVersion": "6.1.0(23)",   // ≤ 设备 OS API
```

打包前先 `hdc shell param get const.ohos.apiversion` 确认设备 API。`targetSdkVersion` 可继续用高版本，不影响安装。
...
LCC 整个 M3-M13（M3-M12 里程碑 + M13 真机部署运维期）路径上踩了 **17+ 真实坑**，分两大类：

| 类型 | 数量 | 自动扫描覆盖 | 必须靠 SKILL 引导 |
| --- | --- | --- | --- |
| ArkTS 语法层（编译期） | 8 | ✅ scan-arkts.sh 覆盖 | — |
| 运行时装配 / 工程层 | 9+ | ❌ grep 扫不出来 | ✅ runtime-pitfalls + 领域 SKILL |
